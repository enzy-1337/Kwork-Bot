...
- `/clear` — очистка контекста диалога в Ollama-топике (forum mode)
- `/bind_ollama` — привязать текущую тему как Ollama-тему (сохраняется и после перезапуска)
- Кнопка `🔄 Обновить объявление` — повторно проверяет страницу заказа, показывает число откликов и статус закрытия/исполнителя (если видно на странице).
- Кнопка `✍️ Откликнуться` — пошагово спрашивает текст, цену и срок, затем пытается отправить отклик на Kwork с вашего аккаунта (нужен `KWORK_COOKIE`).

## Telegram Forum Mode

Для работы в группе с включенными темами (Topics/Forum) задайте:
...
- автоматически создает отдельный топик для каждого нового заказа;
- название топика формирует в виде `[#ID] Название • Бюджет`;
- автоматически создает Ollama-топик при старте;
- в Ollama-топике отвечает на каждое текстовое сообщение;
- в Ollama-топике принимает вложения и отправляет их обратно как `document`.
